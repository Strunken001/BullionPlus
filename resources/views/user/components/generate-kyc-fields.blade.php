    ...
    @foreach ($kyc_fields as $item)
        ...
        @elseif ($item->type == 'file')
            <div class="col-xl-6 col-lg-6 col-md-6 form-group">
                ...
                {{-- Trigger button --}}
                <button type="button" class="btn--base webcam-btn" data-name="{{ $item->name }}">
                    {{ __('Capture from Webcam') }}
                </button>

                <button type="button" class="btn--base capture-btn d-none" data-name="{{ $item->name }}">
                    {{ __('Take') }}
                </button>

                <button type="button" class="btn--base retake-btn d-none" data-name="{{ $item->name }}">
                    {{ __('Retake') }}
                </button>
