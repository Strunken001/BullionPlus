...
@endsection
@section('content')
    <section class="card-details-section ptb-80">
        <div class="container">
            <div class="gift-card-area">
                <div class="row align-items-center mb-30-none">
                    <div class="col-xl-5 col-lg-6 mb-30">
                        <div class="gift-card-img">
                            <img src="{{ $product['logoUrls'][0] ?? '' }}" alt="gift-card">
                        </div>
                    </div>
                    <div class="col-xl-7 col-lg-6 mb-30">
                        <div class="gift-card-deatils">
                            <h3 class="title">{{ __($product['productName']) }}</h3>
                            @if ($product['denominationType'] == 'RANGE')
                                <span class="sub-title">{{ __('Select an amount between') }}
                                    {{ $product['minRecipientDenomination'] }}
                                    {{ $product['recipientCurrencyCode'] }} - {{ $product['maxRecipientDenomination'] }}
                                    {{ $product['recipientCurrencyCode'] }}</span>
                            @endif
                            <div class="row gift-card-input">
                                <div class="row">
                                    <div class="col-xl-12 ">
                                        <div class="row">
                                            @if ($product['denominationType'] == 'RANGE')
                                                <div class="col-xxl-12 col-xl-6 col-lg-8 col-md-12 col-sm-4 form-group">
                                                    ...
                                                    <input type="number" name="amount" class="form--control enter-amount"
                                                    ...
                                                    <div class="radio-wrapper">
                                                        @foreach ($product['fixedRecipientDenominations'] ?? [] as $key => $price)
                                                            <div class="gift-card-radio-item">
                                                                <input type="radio" id="level-{{ $key + 1 }}"
                                                                    name="amount" value="{{ $price }}"
                                                                    onclick="handleRadioClick(this)" class="radio_amount"
                                                                    {{ $key == 0 ? 'checked' : '' }}>
                                                                <label for="level-{{ $key + 1 }}">{{ $price }}
                                                                    {{ $product['recipientCurrencyCode'] }}</label>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                </div>
                                    ...
                                    <div class="col-lg-6 col-md-6 mb-10">
                                        ...
                                        <input type="email" class="form--control" name="receiver_email"
                                            placeholder="{{ __('Enter Receiver Email Address') }}"
                                            value="{{ old('receiver_email') }}">
                                    </div>
                                    <div class="col-lg-6 col-md-6 mb-10">
                                        <label>{{ __('Country') }}</label>
                                        <select class="form--control select2-auto-tokenize country-select" name="country" value="{{ old('country') }}">
                                        </select>
                                    </div>
                                    <div class="col-lg-6 col-md-6 mb-10">
                                    ...
                                    <div class="col-lg-6 col-md-6 mb-10">
                                        ...
                                        <input type="text" name="from_name" class="form--control"
                                            placeholder="{{ __('Your Name') }}" value="{{ old('from_name') }}">
                                    </div>
                                    <div class="col-lg-6 col-md-6 mb-10">
                                        <label>{{ __('Quantity') }} <span>*</span></label>
                                        <input type="number" class="form--control" name="quantity" id="quantityInput" value="{{ old('quantity') }}">
                                    </div>
                                    <div class="col-lg-6 col-md-6 mb-10">
                                        ...
                                        <select class="nice-select" name="currency">
                                            @foreach ($currencies ?? [] as $item)
                                                <option value="{{ $item->code }}" data-id="{{ $item->id }}"
                                                    data-rate="{{ $item->rate }}" data-symbol="{{ $item->symbol }}"
                                                    data-type="{{ $item->type }}"
                                ...
                                <div class="buy-btn pt-3">
                                    <button type="button" class="btn--base buyBtn w-100">{{ __('Buy Now') }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
                                                ...
                                                <div class="preview-list-item">
                                                    <div class="preview-list-left">
                                                        <div class="preview-list-user-wrapper">
                                                            ...
                                                            <div class="preview-list-user-content">
                                                                <span
                                                                    class="last">{{ __('Total Payable Amount') }}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="preview-list-right">
...
@push('script')
    ...
    <script>
        ...
        function senderBalance() {
            ...
            $.ajax({
                type: 'get',
                url: "{{ route('user.gift.card.wallet.balance') }}",
                data: {
                    id: senderCurrencyId
                },
        ...
        function feesCalculation(senderAmount) {
            ...
            if ($.isNumeric(percent_charge) && $.isNumeric(fixed_charge) && $.isNumeric(sender_amount)) {
                ...
                // return total_charge;
                $(".fees-show").html(parseFloat(fixed_charge_calc).toFixed(2) + " " + currencyCode + " + " + parseFloat(
                        percent_charge_calc).toFixed(2) + "% = " + parseFloat(total_charge).toFixed(2) + " " +
                    currencyCode);
                return {
                    total: total_charge,
        ...
        function getExchangeRate() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('.rate-show').html("1 " + receiverCurrencyCode + " = " + parseFloat(exchangeRate).toFixed(
                        4) + " " + senderCurrencyCode);
        ...
        function validateInputField() {
            if (isNaN($("input[name=g_unit_price]").val()) || $("input[name=g_unit_price]").val() == 0 || $(
                    "input[name=g_unit_price]").val() == '') {
