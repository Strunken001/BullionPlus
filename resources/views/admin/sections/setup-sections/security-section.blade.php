...
@section('content')
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#security-item-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i> {{ __("Add Item") }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                ...
                                <td>{{ $item->language->$system_default_lang->highlighted_title ?? "" }}</td>
                                <td>{{ textLength($item->language->$system_default_lang->description ?? "", 60) }}</td>
    ...
    {{-- Security edit modal --}}
    <div id="security-item-edit" class="mfp-hide large">
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="edit-modal-{{ $item->name }}" role="tabpanel" aria-labelledby="edit-modal-{{$item->name}}-tab">
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                ...
                                                'label_after' => '*',
                                                'placeholder' => __('Write Here') . '...',
                                                'name' => $item->code . '_title_edit',
                                                'value' => old(
                                                    $item->code . '_title_edit',
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                ...
                                                'label_after' => '*',
                                                'placeholder' => __('Write Here') . '...',
                                                'name' => $item->code . '_highlighted_title_edit',
                                                'value' => old(
                                                    $item->code . '_highlighted_title_edit',
                                                    $data->value->language->$lang_code->highlighted_title ?? ''),
                                            ])
                                        </div>
                                        <div class="form-group">
...
@push('script')
    ...
    <script>
        openModalWhenError("security-item-add","#security-item-add");
        openModalWhenError("security-item-edit","#security-item-edit");
        ...
        $(".edit-modal-button").click(function(){
            ...
            var editModal = $("#security-item-edit");

            editModal.find("form").first().find("input[name=target]").val(oldData.id);

            $.each(languages,function(index,item) {
                editModal.find("input[name="+item.code+"_title_edit]").val(oldData.language[item.code]?.title);
                editModal.find("input[name="+item.code+"_highlighted_title_edit]").val(oldData.language[item.code]?.highlighted_title);
            ...
            openModalBySelector("#security-item-edit");
