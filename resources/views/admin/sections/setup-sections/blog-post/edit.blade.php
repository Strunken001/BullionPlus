...
@section('content')
    <div class="custom-card">
        ...
        <div class="card-body">
            <form class="card-form" action="{{ setRoute('admin.setup.sections.blog.update',$blog->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row justify-content-center mb-10-none">
                    <div class="col-xl-4 col-lg-4 form-group">
                        @include('admin.components.form.input-file',[
                            ...
                            'old_files'         => old("old_image",$blog->data?->image ?? null),
                        ])
                    </div>
                                ...
                                @foreach ($languages as $item)
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="{{ $item->name }}" role="tabpanel" aria-labelledby="english-tab">
                                        <div class="form-group">
                                            @php
                                                $old_category = $blog->category?->name?->language?->$lang_code?->name ?? "";
                                            @endphp
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input',[
                                                ...
                                                'value'         => old($item->code . "_title",$blog->data?->language?->$lang_code?->title ?? null),
                                            ])
                                        </div>
                                        <div class="form-group">
                                            @include('admin.components.form.input-text-rich',[
                                                ...
                                                'value'         => old($item->code . "_description",$blog->data?->language?->$lang_code?->description ?? null)
                                            ])
                                        </div>
                                        <div class="form-group">
                                            @php
                                                $old_tags = $blog->data?->language?->$lang_code?->tags ?? [];
                                            @endphp
