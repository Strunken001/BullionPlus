    ...
    <div class="support-profile-wrapper">
        <div class="support-profile-header">
            <div class="custom-check-group two mb-0">
                @if (Str::is('admin.*', Route::currentRouteName()))
                    <input class="solve-checkbox" type="checkbox" id="usd"
                        @if ($support_ticket->status == support_ticket_const()::SOLVED) @checked(true) @endif>
                    <label for="usd">{{ __('Mark as Solved') }}</label>
                @else
                    <span class="{{ $support_ticket->stringStatus->class }}">
                        {{ __($support_ticket->stringStatus->value) }}
                    </span>
                @endif
            </div>
        ...
        <div class="support-profile-body">
            ...
            <ul class="support-profile-list">
                ...
                <li>{{ __('Description') }} :<p>{{ $support_ticket->desc }}</p>
                </li>
            ...
            <script>
                ...
                $(".solve-checkbox").change(function() {
                    if ($(this).is(":checked")) {
                        ...
                        var message =
                            `{{ __('Are you sure to mark as solved (Token:') }} <strong>${target}</strong>)? {{ __("Because it's not reversable.") }}`;
