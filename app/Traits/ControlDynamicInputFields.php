...
trait ControlDynamicInputFields
{
    ...
    public function placeValueWithFields($kyc_fields, $form_data, $kyc_verification = false)
    {
        $fields_with_value = [];
        foreach ($kyc_fields ?? [] as $key => $item) {
            if ($item->type == "text" || $item->type == "textarea") {
                ...
                if (is_file($form_file)) {
                    ...
                    $upload_file = upload_files_from_path_dynamic([$get_file_link['dev_path']], $this->file_store_location, null, $kyc_verification);
