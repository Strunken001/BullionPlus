...
class UserCareController extends Controller
{
    ...
    /**
     * Display Phone Unverified Users
     * @return view
     */
    public function phoneUnverified()
    {
        $page_title = __("Phone Unverified Users");
        $users = User::active()->orderBy('id', 'desc')->MobileUnverified()->paginate(12);
        return view('admin.sections.user-care.index', compact(
            'page_title',
            'users'
        ));
    ...
    public function userDetails($username)
    {
        ...
        $completeTransaction = $transactions->complete()->count();
        $completeTransaction = $transactions->complete()->count();
        $completeTicket = $userTicket->solved()->count();
        $allTicket = $userTicket->count();
