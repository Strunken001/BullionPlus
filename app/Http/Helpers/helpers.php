...
function smsVerificationTemplate($user)
{
    ...
    return redirect()->route('user.authorize.mobile', $data['token'])->with(['warning' => [__("Please verify your phone number. Check your inbox to get verification code")]]);
}
...
function generate_google_2fa_auth_qr_image()
{
    $google2FA = new \PragmaRX\Google2FA\Google2FA();
    $secret_key = $google2FA->generateSecretKey();
    $user = auth()->user();
...
{
    try {
        $user = User::where('full_mobile', $request['otp_number'])->firstOrFail();
    } catch (Exception $e) {
        return back()->with(['error' => ['Something went wrong! Please try again']]);
    }
    $data = [
        'user' => $user,
        // 'remember' => $request['stay'] == 'on' ? 1 : 0,
    ];
    return $data;
}

function sendSms($user, $type, $code = null, $shortCodes = [])
{
    $general = BasicSettingsProvider::get();
...
function sendAuthSms($user, $message, $shortCodes = [])
{
    $general = BasicSettingsProvider::get();
    $gateway = $general->sms_config->name;
    $sendSms = new SendSms;
...
function blogCount($category)
{
    return Blogs::where('category_id', $category)->count();
}
...
{
    return Transaction::where('type', $e)->where('user_id', auth()->user()->id)->latest()->paginate(8);
}
...
function paraLimit($e)
{
    $blog_des = strip_tags($e);
    if (strlen($blog_des > 80)) {
        ...
        $blog_des .= "..... <span class='fw-bold'>Read More</span>";
    }
    return $blog_des;
}
...
function get_country_by_phone_code($code)
{
    $countries = json_decode(file_get_contents(resource_path('world/countries.json')), true);
    $iso2 = "";
    foreach ($countries as $item) {
        if ($item['phone_code'] == $code) {
            $iso2 = $item['iso2'];
        }
    }
    if ($iso2 == "") {
        throw new Exception("Sorry, country (" . $code . ") is not available in our list");
    }
    // $phone_code = str_replace("+", "", $phone_code);
    return $iso2;
}
