    ...
    public function array(): array
    {
        return Transaction::with(
            'user:id,firstname,lastname,email,username,full_mobile',
              'currency:id,name',
          )->where('type', PaymentGatewayConst::GIFTCARD)->latest()->get()->map(function($item,$key){
            return [
                'id'                => $key + 1,
                'trx'               => $item->trx_id,
                ...
                'total_charge'      =>  get_amount($item->total_charge,$item->details->charge_info->wallet_currency),
